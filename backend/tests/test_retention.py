...
async def _seed_finished_job(status: str, finished_at: datetime,
                             *, type: str = "index",
                             with_log: bool = True) -> tuple[uuid.UUID, Path]:
    ...
    async with get_session_factory()() as s:
        ...
        job = await jobs_repo.insert_job(
            s, project_id=p.id, type=type, method="fast",
...
async def test_sweep_deletes_expired_succeeded_log(app):
    job_id, log = await _seed_finished_job(
...
async def test_sweep_keeps_fresh_log(app):
    _, log = await _seed_finished_job(
        "succeeded", NOW - timedelta(days=29))
    res = await _sweep(NOW)
    assert res == {"deleted_logs": 0}
    assert log.exists()
...
async def test_sweep_missing_log_file_is_noop(app):
    await _seed_finished_job("succeeded", NOW - timedelta(days=31),
                             with_log=False)
    res = await _sweep(NOW)
    assert res == {"deleted_logs": 0}
...
def test_prune_keeps_newest_two_of_four(tmp_path):
    _make_update_output(tmp_path, "20260801-120000", "20260802-120000",
...
async def test_sweep_all_prunes_and_sweeps(app, tmp_path):
    ...
    _make_update_output(root, "20260801-120000", "20260802-120000",
                        "20260803-120000")
    res = await sweep_all()
    assert res["deleted_logs"] == 1
...
async def test_execute_prunes_after_successful_update(app, monkeypatch):
    ...
    _make_update_output(root, "20260801-120000", "20260802-120000",
                        "20260803-120000")
    monkeypatch.setattr(runner_loop, "IndexRunner",
                        lambda: FakeRunner(RunResult(
...
async def test_ready_reports_disk_fields(client):
    ...
    assert body["disk_ok"] == (body["disk_free_mb"]
                               >= get_settings().disk_watermark_mb)
