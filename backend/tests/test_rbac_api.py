...
async def _grant_global(client, admin, db_session, email, role_ids):
    ...
    r = await client.patch(
        f"/api/admin/users/{uid}", headers=admin, json={"roles": [str(r) for r in role_ids]}
    )
...
async def _project(client, owner_hdr, name="P1") -> str:
    r = await client.post(
        "/api/projects", headers=owner_hdr, json={"name": name, "input_file_type": "text"}
    )
    assert r.status_code == 201
    return r.json()["id"]
...
async def _add_member(client, owner_hdr, pid, uid, role_id):
    r = await client.put(
        f"/api/projects/{pid}/members/{uid}", headers=owner_hdr, json={"role_id": str(role_id)}
    )
    assert r.status_code == 200
...
async def test_maintainer_full_path(client, app, db_session):
    ...
    await _add_member(
        client, alice, pid, await _user_id(db_session, "bob@test.local"), ROLE_ID_MAINTAINER
    )
    ...
    assert r.status_code == 200
    r = await client.post(
        f"/api/projects/{pid}/files", headers=bob, files={"file": ("a.txt", b"hello", "text/plain")}
    )
    assert r.status_code == 201
    ...
    assert r.status_code == 200
    r = await client.post(
        f"/api/projects/{pid}/jobs", headers=bob, json={"type": "index", "method": "standard"}
    )
    assert r.status_code == 201
    job_id = r.json()["id"]
    ...
    r = await client.put(
        f"/api/projects/{pid}/settings", headers=bob, json={"content": "x", "expected_hash": "h"}
    )
    assert r.status_code == 403
    r = await client.post(f"/api/projects/{pid}/dry-run", headers=bob)
    assert r.status_code == 403
    r = await client.patch(
        f"/api/projects/{pid}/env", headers=bob, json={"key": "GRAPHRAG_API_KEY", "value": "v"}
    )
    assert r.status_code == 403
    r = await client.patch(f"/api/projects/{pid}", headers=bob, json={"name": "nope"})
    assert r.status_code == 403
    r = await client.put(
        f"/api/projects/{pid}/members/{await _user_id(db_session, 'alice@test.local')}",
        headers=bob,
...
async def test_viewer_preflight_regression(client, app, db_session):
    ...
    pid = await _project(client, alice)
    await _add_member(
        client, alice, pid, await _user_id(db_session, "carol@test.local"), ROLE_ID_VIEWER
    )
    assert (
        await client.get(f"/api/projects/{pid}/jobs/preflight", headers=carol)
    ).status_code == 200
    assert (
        await client.post(
            f"/api/projects/{pid}/jobs", headers=carol, json={"type": "index", "method": "standard"}
        )
    ).status_code == 403
    assert (
        await client.post(
...
async def test_ops_only_sees_and_acts_everywhere_but_not_users(client, app, db_session):
    ...
    await _grant_global(client, admin, db_session, "dave@test.local", [ROLE_ID_OPS])
    pid = await _project(client, alice)
    ...
    r = await client.patch(f"/api/projects/{pid}", headers=dave, json={"description": "by ops"})
    assert r.status_code == 200
...
async def test_user_admin_only_manages_users(client, app, db_session):
    ...
    await _grant_global(client, admin, db_session, "erin@test.local", [ROLE_ID_USER_ADMIN])
    pid = await _project(client, alice)

    assert (await client.get("/api/admin/users", headers=erin)).status_code == 200
    # no project visibility beyond own memberships (none)
    assert (await client.get("/api/projects", headers=erin)).json() == []
    assert (await client.get(f"/api/projects/{pid}", headers=erin)).status_code == 403
...
async def test_custom_auditor_role_via_api(client, app, db_session):
    ...
    await _grant_global(client, admin, db_session, "frank@test.local", [uuid.UUID(role_id)])
    pid = await _project(client, alice)

    projects = (await client.get("/api/projects", headers=frank)).json()
    assert any(p["id"] == pid for p in projects)
    assert (await client.get(f"/api/projects/{pid}", headers=frank)).status_code == 200
    assert (
        await client.post(
            f"/api/projects/{pid}/files",
            headers=frank,
    ...
    role_id = (
        await client.post(
            ...
            json={
                ...
                "name": "runner",
                "description": "",
                "permissions": ["projects:act_any"],
            },
        )
    ).json()["id"]
    await _grant_global(client, admin, db_session, "gina@test.local", [uuid.UUID(role_id)])
    pid = await _project(client, alice)
...
async def test_userout_carries_roles_and_permissions(client):
    ...
    assert set(me["permissions"]) == {"users:manage", "projects:view_any", "projects:act_any"}
...
async def test_member_contract_and_my_permissions(client, app, db_session):
    ...
    assert members[0]["role_id"] == "00000000-0000-4000-8000-000000000006"
    project = (await client.get(f"/api/projects/{pid}", headers=alice)).json()
