...
async def test_catalog_open_to_authenticated_users(client):
    ...
    assert names == {"user_admin", "ops", "viewer", "maintainer", "editor", "owner"}
    r = await client.get("/api/roles?scope=project", headers=admin)
    assert {role["name"] for role in r.json()} == {"viewer", "maintainer", "editor", "owner"}
...
async def test_admin_crud_and_audit(client, db_session):
    ...
    actions = (
        (await db_session.execute(select(AuditLog.action).where(AuditLog.action == "role.created")))
        .scalars()
        .all()
    )
    assert actions
    ...
    r = await client.patch(
        ...
        headers=admin,
        json={"name": "auditor", "description": "x", "permissions": []},
    )
...
async def test_admin_list_carries_usage_counts(client):
    ...
    counts = {role["name"]: (role["user_count"], role["member_count"]) for role in r.json()}
...
async def test_wrong_scope_atom_rejected(client):
    ...
    r = await client.post(
        "/api/admin/roles",
        headers=admin,
        json={"scope": "global", "name": "bad", "description": "", "permissions": ["project:view"]},
    )
...
async def test_system_role_immutable_via_api(client):
    admin = await _admin(client)
    r = await client.patch(
        f"/api/admin/roles/{ROLE_ID_VIEWER}",
        headers=admin,
        json={"name": "viewer", "description": "", "permissions": []},
    )
    assert r.status_code == 400
    assert r.json()["code"] == "role_is_system"
    r = await client.delete(f"/api/admin/roles/{ROLE_ID_VIEWER}", headers=admin)
    assert r.status_code == 400
    assert r.json()["code"] == "role_is_system"
...
async def test_delete_in_use_conflicts(client, db_session):
    ...
    admin_row = (
        await db_session.execute(select(User).where(User.email == "admin@test.local"))
    ).scalar_one()
    project = Project(
        name="P", slug=f"p-{uuid.uuid4().hex[:8]}", owner_id=admin_row.id, input_file_type="text"
    )
    db_session.add(project)
    await db_session.flush()
    db_session.add(
        ProjectMember(project_id=project.id, user_id=admin_row.id, role_id=uuid.UUID(role_id))
    )
...
async def test_non_admin_crud_forbidden(client):
    ...
    await client.post(
        "/api/admin/users",
        headers=admin,
        json={"email": "alice@test.local", "display_name": "A", "password": "alice-pass-1"},
    )
    alice = await _activate(client, "alice@test.local", "alice-pass-1", "alice-pass-2")
    r = await client.post(
        "/api/admin/roles",
        headers=alice,
        json={"scope": "global", "name": "x", "description": "", "permissions": []},
    )
...
async def test_patch_concurrent_rename_maps_integrity_error(client, monkeypatch):
    ...
    role_id = (
        await client.post(
            "/api/admin/roles",
            headers=admin,
            json={"scope": "global", "name": "auditor", "description": "", "permissions": []},
        )
    ).json()["id"]
    ...
    r = await client.patch(
        f"/api/admin/roles/{role_id}",
        headers=admin,
        json={"name": "renamed", "description": "", "permissions": []},
    )
    assert r.status_code == 409
