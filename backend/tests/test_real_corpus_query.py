...
async def test_real_corpus_query_basic_post_local_stream_rate_limit(
    query_client,  # noqa: F811  (fixture imported above; the param shadows it)
    ws_root,  # noqa: F811  (same)
    monkeypatch,
):
    client = query_client
    ...
    pid = (
        await client.post(
            "/api/projects",
            headers=admin,
            json={"name": "Real Query Corpus", "input_file_type": "text"},
        )
    ).json()["id"]
    ...
    r = await client.post(
        f"/api/projects/{pid}/query", headers=admin, json={"method": "local", "query": QUESTION}
    )
    assert r.status_code == 200, r.text
