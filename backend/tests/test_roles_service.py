...
async def _project(db, owner):
    p = Project(
        name="P", slug=f"p-{uuid.uuid4().hex[:8]}", owner_id=owner.id, input_file_type="text"
    )
    db.add(p)
    await db.flush()
    db.add(ProjectMember(project_id=p.id, user_id=owner.id, role_id=ROLE_ID_OWNER))
    await db.flush()
    return p
...
async def test_create_role_validates_scope(db_session):
    ...
    with pytest.raises(svc.RoleScopeMismatchError):
        await svc.create_role(
            db_session, scope="team", name="x", description="", permissions=[], actor_id=u.id
        )
...
async def test_system_roles_are_immutable(db_session):
    ...
    with pytest.raises(svc.RoleIsSystemError):
        await svc.update_role(
            db_session, role, name="viewer2", description="", permissions=[], actor_id=u.id
        )
    with pytest.raises(svc.RoleIsSystemError):
...
async def test_delete_role_in_use_rejected(db_session):
    ...
    db_session.add(ProjectMember(project_id=p.id, user_id=member.id, role_id=custom.id))
    ...
    other = await svc.create_role(
        db_session, scope="global", name="empty", description="", permissions=[], actor_id=u.id
    )
...
async def test_update_role_dropping_users_manage_guarded(db_session):
    ...
    await svc.update_role(
        db_session, custom, name="helper", description="", permissions=[], actor_id=holder.id
    )
    # now make it the LAST source: drop the direct grant too
    await db_session.execute(
        UserRole.__table__.delete().where(
            UserRole.user_id == holder.id, UserRole.role_id == ROLE_ID_USER_ADMIN
        )
    )
    ...
    with pytest.raises(svc.LastUserManagerError):
        await svc.update_role(
            db_session, custom2, name="last", description="", permissions=[], actor_id=holder.id
        )
