...
MAINTAINER = frozenset({"project:view", "project:edit_content", "project:run_jobs"})
EDITOR = MAINTAINER | {"project:edit_settings"}
OWNER = EDITOR | {"project:manage"}

GLOBAL = {Atom.users_manage, Atom.projects_view_any, Atom.projects_act_any, Atom.projects_create}
...
def test_effective_project_perms_for_my_permissions():
    assert effective_project_perms(OPS, None) == frozenset(a.value for a in PROJECT_ATOMS)
    assert effective_project_perms(AUDITOR, MAINTAINER) == MAINTAINER | {"project:view"}
