...
async def _versions(db_session, pid):
    rows = (await db_session.execute(
        ...
        .order_by(SettingsVersion.id))).scalars().all()
    return rows
...
async def test_get_returns_content_and_hash(client, app):
    ...
    assert body["content_hash"] == hashlib.sha256(
        _settings_path(pid).read_bytes()).hexdigest()
...
async def test_restore_flow_via_versions(client, app, db_session):
    ...
    versions = (await client.get(f"/api/projects/{pid}/settings/versions",
                                 headers=alice)).json()
    assert len(versions) == 2
    v1_detail = (await client.get(
        f"/api/projects/{pid}/settings/versions/{versions[1]['id']}",
        headers=alice)).json()
...
async def test_viewer_can_read_but_not_write(client, app):
    ...
    pid = (await client.post("/api/projects", headers=alice,
                             json={"name": "ViewerRO", "input_file_type": "text"})).json()["id"]
...
async def test_versions_list_newest_first_and_detail_keys(client, app):
    ...
    h1 = (await client.put(f"/api/projects/{pid}/settings", headers=alice,
                           json={"content": "input:\n  type: text\n  base_limit: 5\n",
                                 "expected_hash": got["content_hash"]})).json()["content_hash"]
    h2 = (await client.put(f"/api/projects/{pid}/settings", headers=alice,
                           json={"content": "input:\n  type: text\n  base_limit: 10\n",
                                 "expected_hash": h1})).json()["content_hash"]
    ...
    detail = (await client.get(f"/api/projects/{pid}/settings/versions/{versions[0]['id']}",
                               headers=alice)).json()
