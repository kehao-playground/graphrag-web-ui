...
def _flatten_frames(frames: dict[str, pd.DataFrame]) -> dict[str, dict[int, str | None]]:
    ...
    for name, df in frames.items():
        ...
        texts[name] = entries
        # graphrag labels the text-units frame "units" in some modes while the
        # marker label folds to "text_units" (Task 1 note): expose both keys so
        # citations resolve regardless of which side names it.
        if name == "text_units":
            texts.setdefault("units", entries)
        elif name == "units":
            texts.setdefault("text_units", entries)
    return texts
