...
async def _unique_slug(session: AsyncSession, name: str) -> str:
    ...
    slug = base
    while (
        await session.execute(select(Project.id).where(Project.slug == slug).limit(1))
    ).first() is not None:
...
async def create_project(
    ...
    session.add(ProjectMember(project_id=project.id, user_id=creator.id, role_id=ROLE_ID_OWNER))
    await audit(
        session,
        creator.id,
        "project.created",
...
async def member_perms_for_projects(
    session: AsyncSession, user_id: uuid.UUID, project_ids: list[uuid.UUID]
    ...
    rows = (
        await session.execute(
            ...
            .where(ProjectMember.user_id == user_id, ProjectMember.project_id.in_(project_ids))
        )
    ).all()
...
async def update_project(
    ...
    *,
    ...
    if not changed:  # an empty PATCH is not a write; no audit
        return project
    await audit(session, actor_id, "project.updated", "project", str(project.id), payload=changed)
    await session.commit()
    return project
...
async def set_member(
    ...
    if member is None:
        member = ProjectMember(project_id=project.id, user_id=user_id, role_id=role_id)
        session.add(member)
        await audit(session, actor_id, "member.added", "project", str(project.id), payload=payload)
...
async def remove_member(
    session: AsyncSession, project: Project, user_id: uuid.UUID, actor_id: uuid.UUID | None
) -> None:
    if user_id == project.owner_id:
