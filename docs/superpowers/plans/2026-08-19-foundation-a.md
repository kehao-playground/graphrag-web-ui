...
uv add fastapi 'uvicorn[standard]' 'sqlalchemy[asyncio]' asyncpg alembic \
       pydantic-settings argon2-cffi pyjwt
...
COPY . .
EXPOSE 8000
CMD [".venv/bin/uvicorn", "graphrag_ui.main:app", "--host", "0.0.0.0", "--port", "8000"]
```
...
async def db_session(migrated_db) -> AsyncSession:
    ...
    async with factory() as s:
        yield s
        for t in ["refresh_tokens", "users"]:
            await s.execute(text(f"DELETE FROM {t}"))
        await s.commit()
    await engine.dispose()
```

注意:上面迴圈內刪表請寫完整:`from sqlalchemy import text; await s.execute(text(f"DELETE FROM {t}"))` — 每個測試結束清空,保證隔離(連線內已 commit 的資料)。

`client` fixture:

```python
@pytest.fixture
async def client(migrated_db, monkeypatch, tmp_path):
...
同時把 `main.py` 的 `register_health_routes` 重構為接收 `app`,ready 內執行 `SELECT 1`(透過 `make_engine`),`graphrag_version` 讀 `app.state.graphrag_version`。
...
async def _consume_valid(session: AsyncSession, token: str) -> RefreshToken | None:
...
async def rotate_refresh(session: AsyncSession, token: str) -> tuple[uuid.UUID, str] | None:
    row = await _consume_valid(session, token)
    if row is None:
        return None
    user_id = row.user_id
    await session.delete(row)
    await session.commit()
    return user_id, await issue_refresh_token(session, user_id)


async def revoke_refresh(session: AsyncSession, token: str) -> None:
    h = hashlib.sha256(token.encode()).hexdigest()
    await session.execute(delete(RefreshToken).where(RefreshToken.token_hash == h))
    await session.commit()
...
from graphrag_ui.adapters.db import make_session_factory
...
實作細節:模組級建立 engine/session_factory(讀 `get_settings()`),`get_db` 為 async generator yield session;`get_current_user(creds: Annotated[...Depends(_bearer)], db) -> User`:無 creds → 401;decode JWT(`audience`/`type` 不符或過期 → 401);`db.get(User, uuid.UUID(payload["sub"]))`;`user.is_active` 為假 → 401。

`api/auth_routes.py`:四個端點照 Interfaces 規格,login 成功回傳 `UserOut.model_validate(user, from_attributes=True)` 序列化(`model_config = ConfigDict(from_attributes=True)` 加在 UserOut)。
...
async def test_admin_crud_and_audit(client):
    ...
    # audit rows 已寫入(經由行為驗證:停用後該使用者無法登入)
...
async def test_non_admin_forbidden(client):
    ...
    r2 = await client.get("/api/admin/users", headers=user_tok)
...
`services/users.py` 照 Interfaces 實作(密碼用 `hash_password`;`update_user` 停用時與 `reset_password` 皆呼叫 `revoke_all_for_user`)。路由:`APIRouter(prefix="/api/admin/users", dependencies=[Depends(require_admin)])`;`require_admin = get_current_user` 再檢查 `user.role == "admin"` 否則 403。PATCH 不允許改自己 `is_active=False` 與 `role`(避免鎖死系統,回 400)。每個寫入操作呼叫 `audit(...)`。
...
@pytest.mark.parametrize("action,project_role,expected", [
    # system admin:全通過(project_role 任意)
    *[(a, pr, True) for a in Action for pr in ("owner", "editor", "viewer", None)],
    # 一般使用者 + owner
...
```
  ...
  - models:`Project(id UUID pk, name, slug unique, description, owner_id FK users, input_file_type ∈ text|csv|json, created_at)`、`ProjectMember(project_id, user_id, role ∈ owner|editor|viewer, PK(project_id,user_id))`
  - `services/projects.py`:`create_project(session, name, description, input_file_type, creator) -> Project`、`get_project_role(session, project_id, user_id) -> str | None`、`list_projects(session, user)`、`update_project` / `delete_project`(連同 workspace 目錄,先驗證路徑在 `WORKSPACES_DIR` 內)、`set_member(session, project, user_id, role)`、`remove_member`
...
async def _setup_two_users(client):
    admin = await _login(client, "admin@test.local", "admin-pass-123")
...
async def test_create_project_runs_init_and_adds_owner(client, tmp_path):
    ...
    alice = await _login(client, "alice@test.local", "alice-pass-1")
    ...
    yaml_text = (ws / "settings.yaml").read_text()
    assert "text" in yaml_text                  # input_file_type 已同步
...
async def test_permission_matrix_enforced(client):
    admin = await _setup_two_users(client)
    alice = await _login(client, "alice@test.local", "alice-pass-1")
    bob = await _login(client, "bob@test.local", "bob-pass-1234")
    ...
    bob_id = (await client.get("/api/admin/users", headers=admin)).json()[1]["id"]
...
_FILE_PATTERNS = {"text": "*.txt", "csv": "*.csv", "json": "*.json"}
...
- `create_project`:檢查 `can(user.role, user.is_active, Action.create_project)` → 產生 slug → DB insert(Project + ProjectMember(role="owner"))commit 後呼叫 `initializer.init(ws_path(project_id), input_file_type)`;init 失敗(sqlproc CalledProcessError)→ 回滾:刪除 DB 列、re-raise `HTTPException(500, "graphrag init failed")`
...
- [ ] **Step 6: 跑測試** `uv run pytest tests/test_projects.py -v` → PASS(此測試會真的執行 `graphrag init`,約數秒)
...
test("submits credentials and shows error on 401", async () => {
  ...
  await waitFor(() => expect(screen.getByText(/登入失敗/i)).toBeInTheDocument())
})
```
...
export async function api(path: string, init: RequestInit = {}, retried = false): Promise<Response> {
  const token = useAuth.getState().accessToken ?? (await useAuth.getState().refresh());
...
`components/ProtectedRoute.tsx`:`user == null → <Navigate to="/login" />`;`components/Layout.tsx`:AntD `Layout` 側欄(專案、管理者-使用者(僅 admin)+登出)。
...
(Projects/ProjectDetail/AdminUsers 先放最小佔位組件——「Phase 後續實作」文案;Task 8/9 填實。啟動時 `App` 內 `useEffect` 呼叫 `refresh()` 恢復 session。)
...
注意:backend 容器啟動時需先跑 `alembic upgrade head` 再啟 uvicorn — 修改 backend Dockerfile CMD 為 `sh -c "alembic upgrade head && .venv/bin/uvicorn ..."`。
...
helm template deploy/helm/graphrag-ui --set postgresql.enabled=false \
  --set externalDatabase.url=postgresql+asyncpg://u:p@h:5432/d | grep -c externalDatabase
```

Expected: lint 0 errors; 內建/外部 DB 兩種 render 都成功。

- [ ] **Step 5: Commit** `feat(deploy): docker-compose and helm chart`
...
逐一檢視:`grep -rn "import graphrag\|from graphrag" backend/src/graphrag_ui/{domain,services}` 必須為空;`grep -rn "fastapi\|sqlalchemy" backend/src/graphrag_ui/domain` 必須為空;單檔 >400 行者拆分;services 重複 DB/權限樣式抽 `api/deps.py` helper;刪除未用代碼。發現即修,重跑測試。
...
- **Type consistency:** `can(user_role, is_active, action, project_role)` 於 Task 5 定義、Task 6 使用;`WorkspaceInitializer.init(root, input_file_type)` Task 6 內自洽;前端 `User/Project/Member` 欄位與後端 schemas 對齊。
