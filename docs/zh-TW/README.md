...
**文件**分頁是知識管理者策展專案子集的地方。`input/` 裡的每個檔案都帶有
**索引狀態** —— `new`(未索引)、`modified`(上次索引後有變更)、`indexed`、
`skipped`(上次執行時被靜默略過)、或 `removed`(已從磁碟刪除但索引仍留有
...
- 輸入凍結:`index`/`update` 工作佇列中或執行中時,上傳、單檔/批次刪除、
  `PUT .../settings`、`PATCH .../env` 與 `DELETE .../env/{key}` 會回傳
  **409 `project_indexing`**。
- `FileEntryOut.size` / `.modified_at` / `.sha256` 改為**可為 null**,且清單
  可能包含背後沒有檔案的列(`removed` 文件仍在索引中)。
- 文件分頁:每檔索引狀態、客戶端搜尋與過濾、標籤、附數量與大小確認的
  批次刪除,以及有界的文件預覽(可依段落置中)。

### 測試 —— 檢索測試迴圈

**測試**分頁是檢索測試的迴圈。知識管理者先存好一組題組 —— 臨時查詢的
答案可以一鍵存入題組 —— 然後把**整組題目**當成背景工作,對目前的索引
**重跑一次**。執行的工作在加入佇列時就落實題目清單,因此工作排隊期間
的編輯不會改變實際執行的內容。答案由人工評分(`good` / `fair` /
`poor`,外加評註),在評分矩陣中進行:列是題目譜系,欄是最新的幾次
執行(預設 5 次),「只看退步的」只保留最新評分比上次執行差的題目。
點擊格子會開啟結果抽屜 —— 題目當時的原文、答案、引用與耗時;抽屜開啟
時按 `1`/`2`/`3` 即評分並跳到下一題。再選第二個格子會開啟並排比較,
以**句子**為粒度標示差異 —— 字元級 diff 只會把真正的變動埋進雜訊裡。
編輯跑過的題目會建立新版本;過去的執行仍保留當時實際問出的文字。

**版本資訊 —— 檢索測試(切片二):**

- 測試執行與索引/更新工作**每個專案互斥,雙向皆然**:即使全域
  `MAX_CONCURRENT_JOBS` 額度尚有餘裕,「每專案一個作用中工作」的規則
  仍然成立;工作分頁也會從另一側點名同樣的衝突(HTTP 409 `job_conflict`)。
- 批次執行**在 API 行程內**進行,與互動查詢相同:是連續多次查詢,
  不是新的負載類型 —— 但持續較久,以 `MAX_CONCURRENT_JOBS` 為節流閥。
  它刻意不經過每使用者的互動限流,否則一次 20 題的批次會直接耗盡整個
  限流額度。
- 沒有新的環境變數;界限是領域常數(`MAX_QUESTIONS_PER_SET`、
  `MAX_QUESTION_CHARS`、`MATRIX_DEFAULT_RUNS`)。

### 知識管理者迴圈 —— 打通(切片三)

切片一、二各自做好了零件;切片三把它們之間的迴圈接起來:
**上傳 → 索引 → 測試 → 找出有問題的文件 → 修正 → 重新索引。**
知識管理者上傳文件並執行一次完整 `index`;檢索測試再對該索引提出
題組。當答案不對時,`Sources` 引用會標明出處文件,點一下就打開該
文件的預覽並置中於實際被引用的段落 —— 管理者修正文件後,下一次索引
執行加上重跑題組,就能驗證修正是否生效。總覽頁的行動卡片隨時指出
下一步該做的單一動作,專案列表也會標記缺失(`3 待索引`、「已刪除
文件仍在索引中」),讓默默漂移的專案無法看起來健康。

**版本資訊 —— 知識管理者(切片三):**

- `Sources` 引用現在帶有 `source_name`,在答案產生的當下**隨答案一起**
  解析 —— 臨時查詢與批次執行皆然。**沒有任何端點能在事後**把引用解析
  成文件:引用 id 只對產生它的索引輸出有意義 —— 之後的重建會重新編號,
  延後查找只會靜默地打開*錯的*文件。檔案已被刪除的 `source_name` 會
  渲染成停用的連結並說明文件已刪除,而不是點開一個 404。
- 同樣的保守原則也決定索引失敗或中斷之後的連結可用性:引用連結會保持
  關閉,直到一次成功的索引建立新基準為止。`/health` 的
  `artifacts_stale` 與總覽頁的行動卡片(「索引輸出無法使用」)就是
  使用者看到原因的地方。
