...
**文件**分頁是知識管理者整理專案語料的地方。`input/` 裡的每個檔案都帶有
...
- graphrag 固定在 `==3.1.2`（最新穩定版）。其相依套件 `graphrag-vectors`
  ...
  升級 graphrag 前務必重新確認它讀取的 `settings.yaml` 鍵名（見 `AGENTS.md`
  的 Working Rules）。
...
維護第二組憑證。此模式以部署為單位選用 —— 預設部署的行為與現況完全
...
- **本機登入完全停用** —— `login` / `refresh` / `logout` /
  ...
  偵測模式。
- **標頭信任錨定於共享金鑰** —— `PROXY_AUTH_SECRET`（必填，≥ 32 字元；
...
sequenceDiagram
    ...
    N->>A: 標頭原樣轉送
    A->>A: 常數時間金鑰檢查、JIT 建立使用者<br/>（列於 PROXY_ADMIN_EMAILS 即授予 user_admin + ops）
    A-->>B: 200
...
模式切換注意事項、手動冒煙測試）：**[docs/zh-TW/oauth2-proxy.md](oauth2-proxy.md)**。

## 貢獻與文件

- [CONTRIBUTING.md](../../CONTRIBUTING.md)
- [CHANGELOG.md](../../CHANGELOG.md) —— 各功能切片的版本資訊（英文）
