// en-US catalog (i18n spec §5.1). `satisfies` pins this tree to the
// zh-TW shape — key drift is a compile error.
import type zhTW from "./zh-TW";

type Shape<T> = {
  readonly [K in keyof T]: T[K] extends string ? string : Shape<T[K]>;
};

const enUS = {
  errors: {
    not_indexed: "No index yet — run an indexing job first",
    user_not_found: "User not found",
    forbidden: "Forbidden",
    auth_too_many_attempts: "Too many attempts",
    auth_invalid_credentials: "Invalid email or password",
    auth_invalid_refresh_token: "Invalid refresh token",
    auth_wrong_current_password: "Incorrect current password",
    auth_not_authenticated: "Not authenticated",
    auth_invalid_token: "Invalid or expired token",
    auth_must_change_password: "Password change required",
    auth_user_disabled: "This account is disabled",
    admin_only: "Requires user management permission",
    explore_unknown_table: "Unknown table",
    explore_unsupported_filter: "This table does not support that filter",
    explore_read_failed: "Failed to read index output",
    explore_row_not_found: "Row not found",
    file_name_empty: "Filename must not be empty",
    file_name_too_long: "Filename exceeds 255 characters",
    file_name_unsafe: "Filename must not contain path separators or '..'",
    file_name_leading_dot: "Filename must not start with '.'",
    file_ext_not_allowed:
      "Extension '{{ext}}' is not allowed for input_file_type '{{input_file_type}}'",
    file_too_large: "File exceeds the {{max_mb}} MiB upload limit",
    quota_exceeded: "Project storage quota of {{quota_mb}} MiB exceeded",
    file_not_found: "File not found",
    project_indexing: "An indexing job is running; document and settings changes are paused",
    job_not_found: "Job not found",
    job_conflict: "A job is already active for this project",
    disk_watermark: "Insufficient free disk space",
    job_already_finished: "Job already finished",
    job_invalid_last_event_id: "Invalid Last-Event-ID",
    question_set_not_found: "Question set not found",
    question_not_found: "Question not found",
    question_set_too_large: "A question set holds at most {{max_questions}} questions",
    question_set_empty: "A question set with no questions cannot run",
    test_run_not_found: "Test run not found",
    test_result_not_found: "Test result not found",
    project_not_found: "Project not found",
    init_failed: "graphrag init failed",
    member_owner_protected: "Cannot demote or remove the project owner",
    member_not_found: "Member not found",
    query_rate_limited: "Query rate limit exceeded — try again later",
    query_config_failed: "Failed to load settings",
    query_failed: "Query failed",
    query_interrupted: "Query interrupted",
    settings_conflict: "Settings modified by someone else",
    settings_too_large: "Settings content too large",
    settings_invalid_yaml: "Invalid YAML: {{reason}}",
    settings_invalid_placeholder: "Invalid $ placeholder in settings",
    settings_path_escape: "Settings point graphrag outside the project workspace: {{field}}",
    settings_input_locked: "The input format is fixed at project creation: {{field}}",
    version_not_found: "Version not found",
    env_invalid_body: "Invalid body",
    env_key_value_required: "key and value are required",
    env_value_too_large: "value too large",
    env_invalid_key: "Invalid key: {{key}}",
    env_reserved_key: "Reserved key: {{key}} (process and proxy settings cannot be set per project)",
    env_value_single_line: "value must be a single line",
    env_key_not_found: "key not found",
    env_key_referenced: "settings.yaml still references {{key}} — remove the placeholder first",
    email_registered: "email already registered",
    user_self_change_forbidden: "cannot change your own role or active status",
    user_last_admin_protected: "cannot demote or deactivate the last active admin",
    last_user_manager_protected: "cannot remove the last active user manager",
    role_is_system: "built-in roles are immutable",
    role_in_use: "role is still granted; unassign it first",
    role_name_taken: "a role with that name already exists",
    role_scope_mismatch: "role scope mismatch",
    role_not_found: "role not found",
    role_permissions_invalid: "invalid permission set",
    dry_run_failed: "graphrag dry-run failed",
  },
  common: {
    appName: "GraphRAG Web UI",
    delete: "Delete",
    cancel: "Cancel",
    save: "Save",
    create: "Create",
    submit: "Submit",
    actions: "Actions",
    name: "Name",
    description: "Description",
    email: "Email",
    displayName: "Display name",
    role: "Role",
    roles: "Roles",
    status: "Status",
    createdAt: "Created at",
    notApplicable: "—",
    loading: "Loading…",
  },
  // Built-in role names (backend seed, spec §7): shown wherever a catalog
  // entry renders; custom roles fall back to their raw name.
  roles: {
    user_admin: "User admin",
    ops: "Ops",
    viewer: "Viewer",
    maintainer: "Maintainer",
    editor: "Editor",
    owner: "Owner",
  },
  login: {
    pageTitle: "GraphRAG Web UI Sign in",
    failed: "Sign-in failed — check your email and password",
    passwordLabel: "Password",
    passwordRequired: "Please enter your password",
    submit: "Sign in",
    changeTitle: "Change your password before continuing",
    currentPassword: "Current password",
    newPassword: "New password",
    newPasswordRequired: "Please enter the new password",
    newPasswordMin: "New password must be at least 8 characters",
    wrongCurrent: "Incorrect current password",
    newPasswordInvalid: "New password is invalid (at least 8 characters)",
    emailRequired: "Please enter your email",
    networkError: "Could not reach the server — check your connection and try again",
  },
  projects: {
    loadFailed: "Failed to load projects ({{status}})",
    loadUsersFailed: "Failed to load users ({{status}})",
    createFailed: "Create failed ({{status}})",
    created: "Project created",
    deleteFailed: "Delete failed ({{status}})",
    deleted: "Project deleted",
    inputFormat: "Input format",
    owner: "Owner",
    indexHealth: "Index health",
    healthPending: "{{count}} to index",
    healthRemoved: "Deleted documents still in the index",
    deleteTitle: "Delete project",
    deleteConfirm: "All files in the workspace will be deleted too. Are you sure?",
    pageTitle: "Projects",
    createButton: "New project",
    listLoadFailed: "Unable to load the project list",
    createModalTitle: "Create project",
    nameRequired: "Please enter a name",
    nameMax: "Name must be at most 200 characters",
    inputFormatRequired: "Please select an input format",
  },
  projectDetail: {
    loadMembersFailed: "Failed to load members ({{status}})",
    loadRolesFailed: "Failed to load roles ({{status}})",
    updateMemberFailed: "Failed to update member ({{status}})",
    removeMemberFailed: "Failed to remove member ({{status}})",
    memberRemoved: "Member removed",
    missingId: "Missing project ID",
    loadProjectFailed: "Unable to load the project",
    loadProjectDenied: "Make sure you have access to this project",
    removeTitle: "Remove {{email}}?",
    remove: "Remove",
    overviewTab: "Overview",
    settingsTab: "Settings",
    jobsTab: "Jobs",
    filesTab: "Documents",
    testsTab: "Retrieval tests",
    exploreTab: "Explore",
    membersTab: "Members",
    testsHeading: "Retrieval test workbench",
    groupKnowledgeBase: "Knowledge base",
    groupRetrieval: "Retrieval",
    groupManage: "Manage",
    infoTitle: "Project information",
    slug: "Slug",
    ownerWithNameEmail: "{{name}}({{email}})",
    membersTitle: "Members",
    addMember: "Add member",
    selectUser: "Select a user",
    add: "Add",
  },

  // The knowledge-base health overview (spec §9.3): one action card that
  // is one ordered check, stat tiles, and recent-activity mini-cards.
  overview: {
    heading: "Knowledge base health",
    loadFailed: "Unable to load knowledge-base health",
    statDocuments: "Documents",
    statPending: "Pending index",
    statLastIndex: "Last index",
    statRatings: "Rating summary",
    countDocuments: "{{count}} documents",
    countPending: "{{count}} pending",
    neverIndexed: "Not indexed yet",
    noRuns: "No test runs yet",
    ratingSummary: "Good {{good}}, fair {{fair}}, poor {{poor}} (unrated {{unrated}})",
    lastIndexTitle: "Last index",
    lastIndexLine: "{{type}}, finished {{time}}",
    latestRunTitle: "Latest test run",
    latestRunLine: "Method {{method}}, {{count}} regressed",
    actionActiveJobTitle: "A job is running",
    actionActiveJobDesc: "The {{type}} job is running; health updates when it finishes.",
    actionArtifactsMissingTitle: "Indexed output unavailable",
    actionArtifactsMissingDesc: "The baseline still claims documents are indexed, but the indexed output is missing or was left behind by an index that failed part-way. Citation links stay off until this is repaired. Run a full index.",
    actionNoBaselineTitle: "No index baseline yet",
    actionNoBaselineDesc: "Run a full index to establish a trustworthy baseline; an update will not do.",
    actionRemovedTitle: "Deleted documents still answer queries",
    actionRemovedDesc: "{{count}} deleted documents are still in the index; only a full rebuild clears them.",
    actionStaleTitle: "Documents pending index",
    actionStaleDesc: "{{count}} documents are new or modified; an index update catches up.",
    actionSkippedTitle: "Documents not ingested",
    actionSkippedDesc: "{{count}} documents were not ingested by graphrag; review and re-index.",
    actionRegressionsTitle: "Regressed test results",
    actionRegressionsDesc: "The latest test run has {{count}} regressed questions; review them.",
    actionHealthyTitle: "Knowledge base is healthy",
    actionHealthyDesc: "All documents indexed, no regressed results.",
    linkJobs: "Go to jobs",
    linkPendingFiles: "View pending documents",
    linkSkippedFiles: "View skipped documents",
    linkRegressions: "View regressed results",
    caveatTitleColumn: "Silent-skip detection is off: the skipped count is not evidence either way.",
  },
  adminUsers: {
    updateFailed: "Update failed ({{status}})",
    updated: "User updated",
    resetFailed: "Password reset failed ({{status}})",
    resetDone: "Password has been reset",
    active: "Active",
    inactive: "Disabled",
    edit: "Edit",
    resetPassword: "Reset password",
    disableTitle: "Disable {{email}}?",
    enableTitle: "Enable {{email}}?",
    disableWarning: "Disabling revokes every token issued to this user",
    pageTitle: "User management",
    createButton: "New user",
    listLoadFailed: "Unable to load the user list",
    createModalTitle: "Create user",
    created: "User created",
    emailInvalid: "Invalid email format",
    displayNameRequired: "Please enter a display name",
    displayNameMax: "Display name must be at most 100 characters",
    initialPassword: "Initial password",
    initialPasswordHint: "The user must change it at next sign-in",
    initialPasswordRequired: "Please enter an initial password",
    passwordMin: "Password must be at least 8 characters",
    editTitle: "Edit — {{email}}",
    resetTitle: "Reset password — {{email}}",
    reset: "Reset",
    resetHint: "Resetting revokes all of this user's tokens; they must change the password at next sign-in",
    newPasswordRequired: "Please enter the new password",
    loadRolesFailed: "Failed to load roles ({{status}})",
    rolesPlaceholder: "Select roles",
  },
  adminRoles: {
    title: "Roles", create: "Create Role", edit: "Edit", scope: "Scope",
    scopeGlobal: "Global (users)", scopeProject: "Project (members)",
    permissions: "Permissions", system: "Type", builtin: "built-in",
    usage: "Users / Members", loadFailed: "Failed to load roles ({{status}})",
    saveFailed: "Failed to save role", deleteFailed: "Failed to delete role",
    nameRequired: "Please enter a name",
    deleteConfirm: "Delete role {{name}}?",
    manageWarning: "project:manage grants rename, deletion and member management to any member holding this role.",
  },
  adminAudit: {
    title: "Audit log",
    when: "When",
    actor: "Actor",
    action: "Action",
    target: "Target",
    details: "Details",
    system: "system",
    deletedActor: "(deleted user)",
    filterAction: "Filter by action",
    filterTargetType: "Filter by target type",
    clearFilters: "Clear filters",
    loadFailed: "Failed to load the audit log ({{status}})",
    empty: "No audit entries match these filters",
    retention: "Entries are written by every change; nothing here can be edited or deleted through the UI.",
  },
  // Permission atom labels (spec §7 catalog): shown in the roles table and
  // the scope-aware checkbox editor — labels only, never a gate.
  perms: {
    users_manage: "Manage users", projects_view_any: "See all projects",
    projects_act_any: "Act in all projects", projects_create: "Create projects",
    project_view: "View project", project_edit_content: "Edit documents",
    project_run_jobs: "Run indexing", project_edit_settings: "Edit settings & keys",
    project_manage: "Manage project",
  },
  layout: {
    projects: "Projects",
    adminUsers: "Admin — Users",
    adminRoles: "Admin — Roles",
    adminAudit: "Admin — Audit",
    language: "Language",
    logout: "Log out",
    title: "GraphRAG Web UI",
  },
  files: {
    loadFailed: "Failed to load files ({{status}})",
    loadTagsFailed: "Failed to load tags ({{status}})",
    deleteFailed: "Failed to delete file ({{status}})",
    deleted: "File deleted",
    uploaded: "Uploaded {{name}}",
    uploadFailed: "Upload failed ({{status}})",
    uploadNetworkFailed: "Upload failed",
    size: "Size", modifiedAt: "Modified",
    indexState: "Index state",
    tags: "Tags",
    deleteFileTitle: "Delete {{name}}?",
    uploadHint: "Click or drag files to upload",
    acceptHint: "Accepts {{accept}} only",
    usage: "Used {{used}} / {{quota}}",
    searchPlaceholder: "Search file names…",
    notIndexedBar: "{{n}} documents are not indexed yet",
    goToJobs: "Go to jobs",
    bulkDelete: "Delete selected",
    bulkDeleteTitle: "Delete {{n}} files ({{size}} total)?",
    bulkDeleteDone: "Deleted {{n}} files",
    bulkDeleteFailed: "Bulk delete failed ({{status}})",
    bulkDeletePartial: "Could not delete {{n}} files: {{names}}",
    frozenNotice: "Indexing in progress — document changes are paused",
    previewLoadFailed: "Failed to load preview ({{status}})",
    previewMatch: "Match",
    previewMatchHit: "Yes",
    previewMiss: "No",
    previewOffset: "Window offset",
    previewTotal: "Total size (bytes)",
    state: {
      new: "New", modified: "Modified", removed: "Removed",
      indexed: "Indexed", skipped: "Skipped",
    },
    stateSentence: {
      new: "Not in the index yet; run an index or update to include it.",
      modified: "Changed since the last index run; an update will sync it.",
      removed: "No longer in input/ but its old content is still indexed; only a full rebuild clears it.",
      indexed: "In the current baseline; file content matches the index.",
      skipped: "Silently skipped by the last index run; it never reaches query results.",
    },
    ingestCheck: {
      off: "Silent-skip detection is off",
      unavailable_no_baseline: "This project has no index baseline yet, so skipped documents cannot be detected.",
      unavailable_not_indexed: "The index output (output/) is missing; run a full index again.",
      unavailable_title_column: "This index was built with the title column, so file content cannot be compared.",
    },
  },
  jobs: {
    typeIndex: "Index", typeUpdate: "Update",
    methodStandard: "Standard", methodFast: "Fast",
    durationSeconds: "{{s}}s",
    durationMinutes: "{{m}}m",
    durationMinutesSeconds: "{{m}}m {{s}}s",
    durationHours: "{{h}}h",
    durationHoursMinutes: "{{h}}h {{m}}m",
    loadPreflightFailed: "Failed to load preflight info ({{status}})",
    loadFailed: "Failed to load jobs ({{status}})",
    startFailed: "Failed to start job ({{status}})",
    queued: "Job queued",
    cancelFailed: "Cancel failed ({{status}})",
    cancelRequested: "Cancellation requested",
    confirmTitle: "Start {{type}}?",
    lastRun: "Last run: about {{s}}s, {{docs}} documents",
    noRuns: "No runs for this project yet",
    cacheOver: "Cache over quota ({{used}} MB / {{quota}} MB) — clean it up first",
    diskLow: "Low disk space ({{free}} MB free, below the {{watermark}} MB watermark) — the job may fail",
    start: "Start",
    type: "Type", method: "Method",
    exitCode: "Exit code", queuedAt: "Queued at", duration: "Duration",
    logs: "Logs",
    cancelJobTitle: "Cancel this job?", confirmCancel: "Yes, cancel",
    startIndex: "Start indexing",
    logsTitle: "Job logs",
  },
  client: {
    loadTableFailed: "Failed to load table ({{status}})",
    loadDetailFailed: "Failed to load row detail ({{status}})",
    loadGraphFailed: "Failed to load graph ({{status}})",
  },
  query: {
    methodLocal: "Local", methodGlobal: "Global",
    methodDrift: "DRIFT", methodBasic: "Basic",
    failedRetry: "Query failed — try again later",
    citations: "Citations ({{count}})",
    // A citation whose document no longer exists (spec §7.4): the link is
    // disabled rather than left to 404.
    sourceRemoved: "This document was removed",
    timings: "frames {{frames}}ms · search {{search}}ms · citations {{citations}}ms · total {{total}}ms",
  },
  workbench: {
    modeMatrix: "Rating matrix",
    modeAdhoc: "Ad-hoc query",
    loadSetsFailed: "Failed to load question sets ({{status}})",
    loadQuestionsFailed: "Failed to load questions ({{status}})",
    loadMatrixFailed: "Failed to load the rating matrix ({{status}})",
    setPlaceholder: "Choose a question set",
    rerunSet: "Re-run the set",
    launchTitle: "Re-run the question set",
    launchSummary: "Will run {{count}} questions with method:{{method}}",
    start: "Start run",
    queued: "Test run queued",
    startFailed: "Failed to start the test run ({{status}})",
    typeIndex: "An indexing job",
    typeUpdate: "An update job",
    typeTestRun: "A test run",
    jobRunning: "{{type}} is running — start the test run once it finishes",
    regressionsOnly: "Regressions only",
    questionColumn: "Question",
    notRun: "Not run",
    errored: "Error",
    unrated: "Unrated",
    ratingGood: "Good",
    ratingFair: "Fair",
    ratingPoor: "Poor",
    noRuns: "No test runs yet — re-run the set to build the matrix",
    adhocPlaceholder: "Type a question",
    adhocRun: "Run",
    saveAsQuestion: "Save as question",
    saveTitle: "Save as question",
    saveOk: "OK",
    saved: "Saved into the question set",
    saveFailed: "Failed to save the question ({{status}})",
    // Task 8: cell drawer, keyboard rating, run diff, question editing.
    resultTitle: "Result",
    resultPosition: "Result {{current}} of {{total}}",
    resultMissing: "This result is not in the run",
    loadResultsFailed: "Failed to load results ({{status}})",
    ratingHint: "Press 1=good 2=fair 3=poor to rate and advance",
    noteLabel: "Note",
    notePlaceholder: "Why this score? (optional)",
    rateFailed: "Failed to rate ({{status}})",
    questionsTitle: "Questions ({{count}})",
    questionsEmpty: "No questions in this set yet",
    editQuestion: "Edit question",
    editQuestionAria: "Edit question: {{text}}",
    editForkWarning: "This creates a new version — past runs keep the original question text",
    editContinue: "Continue editing",
    questionUpdated: "Question updated",
    compareTitle: "Side-by-side comparison",
    compareHint: "One cell selected — click another to compare side by side",
    diffLeftOnly: "Left only",
    diffRightOnly: "Right only",
  },
  explore: {
    columns: {
      id: "Internal ID", human_readable_id: "ID", title: "Title", type: "Type",
      frequency: "Frequency", degree: "Degree", description: "Description",
      text_unit_ids: "Text unit IDs", source: "Source", target: "Target",
      weight: "Weight", combined_degree: "Combined degree", community: "Community",
      level: "Level", parent: "Parent", children: "Children", size: "Size",
      entity_ids: "Entity IDs", relationship_ids: "Relationship IDs", period: "Period",
      rank: "Rank", rank_score: "Rank score", summary: "Summary",
      full_content: "Full content", findings: "Findings",
      created_at: "Created at", updated_at: "Updated at",
      n_tokens: "Token count", document_id: "Document ID", text: "Text",
      raw_data: "Raw content", creation_date: "Creation date",
    },
    tableEntities: "Entities", tableRelationships: "Relationships",
    tableCommunities: "Communities", tableCommunityReports: "Community reports",
    tableTextUnits: "Text units", tableDocuments: "Documents",
    staleWarning: "Indexing in progress — results may be incomplete",
    truncatedWarning: "Showing the {{count}} most connected entities — the graph was capped for this response",
    modeGraph: "Graph", modeTable: "Table",
    search: "Search", searchPlaceholder: "Search keywords",
  },
  graph: {
    minDegree: "Min degree",
    searchNodes: "Search nodes", searchNodesPlaceholder: "Search node name",
    empty: "No nodes to display",
  },
  settings: {
    loadFailed: "Unable to load settings",
    loadVersionsFailed: "Unable to load versions",
    loadEnvFailed: "Unable to load environment variables",
    loadVersionFailed: "Unable to fetch version content",
    saveFailed: "Save failed",
    saved: "Saved",
    dryRunFailed: "dry-run failed",
    envSetFailed: "Setting failed",
    envSet: "Set",
    envDeleteFailed: "Delete failed",
    envDeleted: "Deleted",
    yamlUnparsable: "YAML cannot be parsed — switch back to YAML mode, fix it, then save",
    yamlNotObject: "The settings YAML is empty or not an object — form editing is unavailable; use YAML mode",
    saveSettings: "Save settings",
    modelTitle: "Model (model)",
    embeddingModelTitle: "Embedding model (model)",
    inputTitle: "input (locked at creation, spec §6.5)",
    chunkTitle: "Chunking",
    yamlMode: "YAML",
    formMode: "Form",
    envKeyPlaceholder: "Key (e.g. GRAPHRAG_API_KEY)",
    envValuePlaceholder: "Value",
    conflictTitle: "Settings changed by someone else (conflict)",
    reload: "Reload",
    overwrite: "Overwrite",
    serverContent: "Content on the server",
    yourContent: "Your content",
    versionHistory: "Version history",
    view: "View",
    restore: "Restore",
    versionTitle: "Version #{{id}}",
    close: "Close",
    dryRunButton: "Validate settings (dry-run)",
    dryRunPassed: "dry-run passed",
    envTitle: "Environment variables (workspace .env)",
    envSetButton: "Set",
  },
} satisfies Shape<typeof zhTW>;

export default enUS;
