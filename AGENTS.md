...
cd backend && uv run pytest -v          # 575 tests with GRAPHRAG_API_KEY (569 fast); 6 slow tests fork the real graphrag CLI (4 need the key, skipped without it); fast only: uv run pytest -m "not slow"
